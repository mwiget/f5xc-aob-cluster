resource "restapi_object" "subnets_static" {
  ...
  data         = jsonencode({
    "metadata": {
      "name": format("%s-%s-%s-static", var.f5xc_cluster_name, split(".", each.value)[0], split(".", each.value)[1]),
...
#output "subnets_static" {
#  value = restapi_object.subnets_static
#}
